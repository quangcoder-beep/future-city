 ...
 */
@Repository
@DependsOn({"itemRepository", "shopRepository", "userRepository"})
public class OrderRepository {
    ...
    @Autowired
    private com.example.server_spring.services.PlayerService playerService;
    ...
    public int completeDelivery(int courierId, int orderId) {
        try {
            ...
            // Increment deliveries and reputation (Aggressive Reputation 100 max)
            PlayerRepository.PlayerData courierData = playerRepo.getPlayerData(courierId);
            if (courierData != null) {
                int newDeliveries = courierData.deliveries + 1;
                float newReputation = Math.min(100.0f, courierData.reputation + 0.1f);
                playerRepo.updateStats(courierId, newDeliveries, newReputation);
                
                // Update in-memory player if online
                com.example.server_spring.entity.ServerPlayer sp = playerService.get(courierId);
                if (sp != null) {
                    sp.setCredits(newBal);
                    sp.setDeliveries(newDeliveries);
                    sp.setReputation(newReputation);
                }
            }
