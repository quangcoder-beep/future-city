...
import com.futurecity.shared.packets.resonse.WorldTimeResponse;
...
public class GameLoopService {
    ...
    private float worldTime = 800.0f; // 08:00 AM sáng
    ...
    public void tick() {
        ...
        try {
            ...
            // 4. Thời gian thế giới (7 phút thực = 24h game)
            // 2400 units = 24h. 1s = 2400 / 420s = 5.714 units/s.
            worldTime += delta * 5.714f;
            if (worldTime >= 2400f)
                worldTime -= 2400f;
            ...
            // 6. Broadcast World Time (mỗi 5 giây một lần để test nhanh)
            if (tickCount % 300 == 0) {
                broadcastWorldTime();
            }
        } catch (Exception e) {
            System.err.println("[SERVER FAULT] Unexpected exception in GameLoop tick: " + e.getMessage());
            e.printStackTrace();
            if (serverStatsManager != null) {
                serverStatsManager.logError("CRITICAL", "GameLoop", "TICK Crash: " + e.getClass().getSimpleName() + " - " + e.getMessage(), null);
            }
            // Khôi phục lastTickTime để tránh frame lag đột biến ở tick kế tiếp
            lastTickTime = System.nanoTime();
        }
    }

    private void broadcastWorldTime() {
        WorldTimeResponse timePacket = new WorldTimeResponse();
        timePacket.time = worldTime;
        playerService.broadcastTCP(timePacket);
    }
