...
public class AuthenticationController {
    ...
    public ResponseEntity<LoginResponse> login(@RequestBody LoginRequest request) {
        ...
        // Nếu thông báo chứa từ khóa "KHÓA", trả về 403 Forbidden
        if (resp.message != null && resp.message.contains("KHÓA")) {
    ...
    public ResponseEntity<LoginResponse> register(@RequestBody LoginRequest request) {
        ...
        if (authService.register(request.username, request.password, request.nickname)) {
            resp.status = "SUCCESS";
