...
import com.futurecity.shared.exceptions.PathNotFoundException;
import java.util.*;
...
public class NavigationGrid {
    ...
    public List<Vector3> findPath(float startX, float startZ, float targetX, float targetZ) {
        ...
        // Boundary Check (ngăn điểm đến là vách tường hoặc ngoài đồ thị dẫn đến Infinite Loop)
        if (!isWalkable(goal.x, goal.z)) {
            throw new PathNotFoundException("GPS: Điểm đến (" + targetX + ", " + targetZ + ") nằm ngoài khu vực có thể di chuyển (Obstacle).");
        }
        ...
        int maxSearchNodes = 100000; // Giới hạn chống treo máy
        int nodesSearched = 0;

        while (!openSet.isEmpty()) {
            if (nodesSearched >= maxSearchNodes) {
                throw new PathNotFoundException("GPS: Vượt quá giới hạn tìm kiếm (" + maxSearchNodes + " nodes). Path quá phức tạp hoặc không có lối vào.");
            }
