...
public class MapManager {
    ...
    public void prepareMap() {
        ...
        // Using shared MapLoader for perfect parity with Server-side pathfinding
        com.futurecity.shared.utils.MapLoader.MapData mapData = com.futurecity.shared.utils.MapLoader.extractMapData(cityAsset, scale);
        
        float gridSize = 2.0f; 
        float padding = 500.0f; // Synchronized 500m padding for GPS stability

        BoundingBox totalBounds = mapData.fullBounds;
        float minX = totalBounds.min.x - padding;
        float minZ = totalBounds.min.z - padding;
        float maxX = totalBounds.max.x + padding;
        float maxZ = totalBounds.max.z + padding;

        int gridW = (int) ((maxX - minX) / gridSize) + 2;
        int gridH = (int) ((maxZ - minZ) / gridSize) + 2;

        navGrid = new NavigationGrid(gridW, gridH, gridSize, minX, minZ);

        // Authoritatively bake roads from shared MapData
        for (com.futurecity.shared.utils.MapLoader.RoadData rd : mapData.roads) {
            bakeRoadToGrid(rd.bounds);
        }

        // Still do the recursive walk for Interactive objects (NPCs, Shops)
    ...
    private void walkNodesRecursive(Node node) {
        ...
        if (nodeId != null) {
            ...
            // Process terrain types for Minimap visualization (GUI only)
            if (type == MapNodeType.ROAD || type == MapNodeType.BRIDGE) {
                ...
                roads.add(road);
                // We no longer bake here to avoid scale-timing issues. 
                // Grid is now populated authoritatively in prepareMap via MapLoader.
    ...
    /**
     * Mark the position of an object in the NavigationGrid using its world bounds.
     */
    private void bakeRoadToGrid(BoundingBox bounds) {
        if (navGrid == null)
            return;

        // Use world bounds directly as they are expected to be in synced world coordinates
        ...
        for (int x = minX; x <= maxX; x++) {
            for (int z = minZ; z <= maxZ; z++) {
                navGrid.setCell(x, z, NavigationGrid.ROAD);
            }
        }
    }
    ...
    /**
     * Finds a path locally on the client using the NavigationGrid.
     * Avoids a round-trip to the server, drastically reducing GPS lag.
     */
    public List<Vector3> findPath(Vector3 start, Vector3 target) {
        if (navGrid == null) {
            System.err.println("[MapManager] findPath: NavigationGrid is not initialized!");
            return null;
        }
        List<Vector3> path = navGrid.findPath(start.x, start.z, target.x, target.z);
        if (path == null) {
            System.out.println("[MapManager] findPath FAILED: (" + start.x + "," + start.z + ") -> (" + target.x + "," + target.z + ")");
        } else {
            System.out.println("[MapManager] findPath SUCCESS: " + path.size() + " nodes");
        }
        return path;
    }
}
