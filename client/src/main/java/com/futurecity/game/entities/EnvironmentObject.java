...
public class EnvironmentObject extends GameObject {
    ...
    public EnvironmentObject(ModelInstance instance, String nodeId) {
        ...
        if (node != null) {
            ...
            // Override láº¡i logic tĂ­nh bounds Ä‘á»ƒ bao gá»“m cáº£ node transform vĂ  city scale
            node.calculateBoundingBox(localBounds);
            // worldBounds = (localBounds * node.globalTransform) * instance.transform
            worldBounds.set(localBounds).mul(node.globalTransform).mul(instance.transform);

            // Cáº­p nháº­t vá»‹ trĂ­ trung tĂ¢m trong tháº¿ giá»›i
