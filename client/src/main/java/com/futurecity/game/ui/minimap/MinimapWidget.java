...
public class MinimapWidget extends Group {
    ...
    // Flag để đánh dấu người dùng đã chủ động tắt GPS, tránh bị server updates ép bật lại
    ...
    public MinimapWidget(MainCharactor player, MapManager mapManager, Skin skin,
        ...
        if (networkManager != null) {
            networkManager.setPathfindingListener(path -> {
                ...
                if (manualCanceled) return;

                // System.out.println("CLIENT: Path received from Server. Nodes: " + (path != null ? path.size() : 0));
    ...
    public void draw(Batch batch, float parentAlpha) {
        ...
        // Chuyá»ƒn Ä‘á»•i tá»a Ä‘á»™ Ä‘á»ƒ váº½ Ä‘Ăºng vá»‹ trĂ­
        float x = getX();
        float y = getY();
    ...
    public void setNavigationTarget(Vector3 targetWorldPos, boolean manual) {
        ...
        if (networkManager != null) {
            // System.out.println("CLIENT: GPS On " + (manual ? "Manual" : "Auto") + " to: " + targetWorldPos);
            networkManager.sendPathfindingRequest(targetWorldPos);
        }
    }
